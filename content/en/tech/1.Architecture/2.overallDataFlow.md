...
1. Ingestor server subscribe all raw data from Azure IoT Hub.
   ...
   - If the raw data is real-time data, It will parse the data separately and then publish it to Redis (Stream).
...
RT -->|Stream XAdd Parsed RT Data To redis| RTRedis
...
- Publishes messages through Redis(Stream) for immediate delivery to WebSocket clients.
...
- WebSocket Service
- Subscribes to real-time parsed data from Redis (Stream).
...
RTRedis -->|Stream XRead Parsed Real-time Data| WS
